...
// Bandeau logo standard pour tous les emails Compo (même bloc que le mail de
// rappel d'agenda) — titreHtml peut contenir un emoji/icône avant le texte, ex.
// osEnteteEmailLogo('🔕 Rappel d\'inactivité'). Une fonction dédiée plutôt qu'un
// bout de HTML recopié dans chacun des ~40 envois d'email, pour que l'identité
// visuelle reste cohérente et facile à changer d'un seul endroit.
function osEnteteEmailLogo(titreHtml){
  return '<div style="background:#1A1A2E;padding:1.2rem 1.5rem;text-align:center;">'
    +'<img src="https://ipsummedia.fr/assets/logo.png" alt="Ipsum Média" style="height:84px;margin-bottom:0.6rem;">'
    +'<h1 style="color:white;font-size:1rem;margin:0;">'+titreHtml+'</h1>'
    +'</div>';
}

// Nouveau modèle d'email (validé par Tom pour l'agenda, 2026-09-25) : logo sur fond
// blanc, pastille de couleur, titre, texte, carte optionnelle, boutons, pied de page
...
function _emailLigneInfo(label, valeur){
  return '<tr><td style="padding:7px 0;border-top:1px solid '+EMAIL_COUL.bord+';width:70px;vertical-align:top;font:600 10px/1.6 '+EMAIL_POLICE+';letter-spacing:0.08em;text-transform:uppercase;color:'+EMAIL_COUL.gris+';">'+label+'</td>'
    +'<td style="padding:7px 0;border-top:1px solid '+EMAIL_COUL.bord+';vertical-align:top;font:400 14px/1.5 '+EMAIL_POLICE+';color:'+EMAIL_COUL.encre+';">'+valeur+'</td></tr>';
}
function _emailBadgeDate(d){
  var mois = d.toLocaleDateString('fr-FR',{month:'short'});
  return '<table role="presentation" cellpadding="0" cellspacing="0" style="width:56px;text-align:center;">'
    +'<tr><td style="background:'+EMAIL_COUL.rouge+';color:#fff;border-radius:7px 7px 0 0;font:700 10px/1 '+EMAIL_POLICE+';letter-spacing:0.08em;text-transform:uppercase;padding:5px 0;">'+esc(mois)+'</td></tr>'
    +'<tr><td style="background:#F5F4F2;border:1px solid '+EMAIL_COUL.bord+';border-top:none;border-radius:0 0 7px 7px;font:700 24px/1 Arial, sans-serif;color:'+EMAIL_COUL.encre+';padding:8px 0 7px;">'+d.getDate()+'</td></tr></table>';
}
function _emailBouton(label, url, secondaire){
...
function _emailCompo(o){
  ...
  return '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#F4F2EF;"><tr><td align="center" style="padding:24px 12px;">'
    +'<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#fff;border-radius:12px;border:1px solid '+EMAIL_COUL.bord+';">'
...
// Texte variable (titre, source, lieu...) inséré dans un message Google Chat : Chat
// interprète * _ ~ ` comme de la mise en forme — un titre qui en contient décalait tout
// le gras/italique du message. On les retire, et on écrase les retours à la ligne.
function _chatSansMiseEnForme(s){
...
function ticketCreer(){
  ...
  .then(function(data){
    ...
    var html = '<div style="font-family:sans-serif;max-width:600px;margin:0 auto;">'+
      osEnteteEmailLogo('Nouvelle demande d assistance')+
      '<div style="padding:1.2rem 1.5rem;">'+
  ...
  fetch(SB_URL+'/rest/v1/tickets?id=eq.'+ticket.id, {
    ...
    .then(function(data){
      ...
      if(membre && membre.email){
        var htmlMembre = '<div style="font-family:sans-serif;max-width:600px;margin:0 auto;">'+
...
function osLancer(){
  ...
  // Mode allégé / police adaptée / taille de texte / mode malvoyant : appliqués beaucoup
  // plus tôt désormais, tout en haut de demarrer() — voir ce fichier, section INIT OS —
  // pour couvrir aussi l'écran de connexion. Pas la peine de les rappeler ici.
