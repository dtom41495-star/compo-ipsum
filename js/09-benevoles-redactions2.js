...
function osBenevolesDashRender(){
  ...
  Promise.all(fetches).then(function(results){
    ...
    sh+=statCard(artCorrection.length,'Au SR','#1A5276');
    ...
    articles.slice(0,60).forEach(function(a){
      ...
      if((a.statut==='valide'||a.statut==='valide_central') && _osArticlePubliable(a)) eh+='<button onclick="publierArticle(\''+esc(a.id)+'\')" style="font-family:Space Mono,monospace;font-size:0.58rem;padding:2px 7px;border:0.5px solid #155724;border-radius:4px;background:#D4EDDA;color:#155724;cursor:pointer;">Mettre en ligne</button>';
      eh+='</td></tr>';
    });
    eh+='</tbody></table></div>';
...
function osBenvEmailCandidature(annonceId, annonceTitre, candidatPrenom){
  ...
  .then(function(admins){
    ...
    var corps = '<div style="font-family:sans-serif;max-width:560px;margin:0 auto;">'
      +osEnteteEmailLogo('Ipsum Média · Compo OS')
  ...
  Promise.all([
    ...
    function envoyerSujetProchain(idx){
      ...
      var html = '<div style="font-family:sans-serif;max-width:600px;margin:0 auto;">'
        +osEnteteEmailLogo('📌 Nouveaux sujets à rédiger')
        +'<div style="padding:1.2rem 1.5rem;">'
        +'<p style="color:#666;font-size:0.85rem;margin:0 0 0.8rem;">Bonjour '+esc(m.prenom||'')+' — '+sujets.length+' nouveau(x) sujet(s) disponible(s)</p>'
        +sujetsHtml+'</div>'
  ...
  .then(function(data){
    ...
    .then(function(destinataires){
      ...
      function envoyerProchain(idx){
        ...
        var html = '<div style="font-family:sans-serif;max-width:600px;margin:0 auto;">'
          +osEnteteEmailLogo('📢 Nouvelle annonce de recrutement')
          +'<div style="padding:1.2rem 1.5rem;">'
...
function osRedacChargerSujets(zone, roleRedac, callback){
  ...
  fetch(url,{headers:authH}).then(function(r){return r.json();}).then(function(sujets){
    ...
    // Corrige le badge du rail avec le vrai compte de CETTE rédaction — le rail n'est
    // construit qu'une fois (voir osRedactionsRenderAvecOnglets), donc à l'ouverture de
    // l'onglet Sujets juste après avoir changé de rédaction, le badge affichait encore
    // le compte de la rédaction précédemment consultée (window._sujetsData pas encore à
    // jour à ce moment-là). Recrutement/Communiqués ont déjà ce correctif, Sujets non :
