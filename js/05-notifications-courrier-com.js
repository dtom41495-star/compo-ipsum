...
function osCloseWindowForce(pageId){
  ...
  // Ma rédac fermée sur l'onglet Sujets/Recrutement : ce qui y était affiché a été vu,
  // sinon l'icône du dock se remettrait à clignoter pour des choses qu'on vient de lire.
  if(pageId === 'redactions'){
  ...
  if(pageId === 'redactions') osRedacMajClignotement();
  // Corriger/envoyer un article change l'état "à corriger" : le vérifier tout de suite
  // à la fermeture de l'éditeur ou de Mes articles plutôt qu'au prochain passage du minuteur.
  if(pageId === 'mes-articles' || pageId === 'redaction' || pageId === 'app-correction'){
    osMesArticlesMajClignotement();
    setTimeout(osMesArticlesChargerACorriger, 1500);
  }
...
function osBuildDesktopIcons(){
  ...
  // Icône Veille toujours présente sur le bureau (Tom veut qu'elle soit visible
  // directement, pas seulement dans le dock)
  if(role === 'admin' || role === 'redacteur' || role === 'correcteur'){
    ...
    veilleImg.style.background = 'linear-gradient(135deg,#0C5460,#149EB2)';
    veilleImg.style.color = 'white';
    veilleImg.innerHTML = '<i class="ti ti-rss"></i>';
    var veilleLabel = document.createElement('div');
    veilleLabel.className = 'desktop-icon-label';
    veilleLabel.textContent = 'Veille';
    veilleIcon.appendChild(veilleImg);
    veilleIcon.appendChild(veilleLabel);
    container.appendChild(veilleIcon);
  }
  ...
  // Raccourci Tchat — ouvre Google Chat (l'équipe y a remplacé la messagerie interne).
  // Logo Google Chat en SVG vectoriel (version mai 2026, Wikimedia Commons), ids du
  // masque/dégradé préfixés pour ne jamais entrer en collision avec un autre SVG inline.
  var chatIcon = document.createElement('div');
  chatIcon.className = 'desktop-icon';
  chatIcon.title = 'Google Chat';
  chatIcon.onclick = function(){ window.open('https://chat.google.com/','_blank'); };
  var chatImg = document.createElement('div');
  chatImg.className = 'desktop-icon-img';
  chatImg.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 192 192" width="42" height="42" fill="none" role="img" aria-label="Google Chat">'
    ...
    +'<mask id="gchat-m" width="176" height="129" x="8" y="48" maskUnits="userSpaceOnUse" style="mask-type:alpha"><path fill="#0EBC5F" d="M133 48c28.167 0 51 22.834 51 51 0 28.167-22.833 51-51 51H96.722l-39.428 25.896c-3.99 2.62-9.294-.242-9.294-5.015V148.81C25.121 143.78 8 123.39 8 99c0-28.166 22.834-51 51-51h74z"/></mask>'
    +'<g mask="url(#gchat-m)"><rect width="160" height="96" x="16" y="28" fill="#0EBC5F" rx="48"/><rect width="160" height="96" x="16" y="28" fill="url(#gchat-g)" rx="48"/>'
    +'<path stroke="#fff" stroke-linecap="round" stroke-width="12" d="M62 94s8.84 18 34 18 34-17.182 34-17.182"/></g>'
    +'<defs><linearGradient id="gchat-g" x1="96" x2="96" y1="28" y2="124" gradientUnits="userSpaceOnUse"><stop offset=".09" stop-color="#94D4FF"/><stop offset=".28" stop-color="#78C9FF"/><stop offset=".88" stop-color="#01AE58" stop-opacity="0"/></linearGradient></defs>'
    +'</svg>';
  var chatLabel = document.createElement('div');
  chatLabel.className = 'desktop-icon-label';
  chatLabel.textContent = 'Tchat';
...
function _creerAppTuile(app, epingle){
  ...
  var BADGE_APPS = ['tickets','tableau','app-correction','cps-admin','projets','veille'];
