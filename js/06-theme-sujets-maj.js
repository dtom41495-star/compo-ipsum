...
// ===== MA RÉDAC : CLIGNOTEMENT DE L'ICÔNE DU DOCK =====
// L'icône clignote (voir .notif-clignote dans le CSS) quand il reste du non lu — sujets,
// recrutement ou communiqués — ET que la fenêtre Ma rédac n'est pas ouverte à l'écran.
// Le "lu" est celui déjà utilisé par les badges du rail (mêmes clés localStorage) : les
// ids non lus sont chargés en arrière-plan (les globales habituelles ne se remplissent
// qu'à l'ouverture de l'appli), puis comparés aux listes "vus" toutes les quelques
// secondes, ce qui couvre lecture, fermeture et réduction sans dépendre d'un hook précis.
var _redacNonLusIds = { sujets:[], recrutement:[], cps:[] };
var _redacClignoteInit = false;

function _osLireListeVus(cle){
  try{ var v = JSON.parse(localStorage.getItem(cle)||'[]'); return Array.isArray(v) ? v : []; }
  catch(e){ return []; }
}

function osRedacResteDuNonLu(){
  if(!_redacNonLusIds) return false;
  var lu = {
    sujets: _osLireListeVus('compo_os_sujets_vus'),
    recrutement: _osLireListeVus('compo_os_recrutement_vus'),
    cps: _osLireListeVus('ipsum_cps_vus')
  };
  return Object.keys(_redacNonLusIds).some(function(type){
    return _redacNonLusIds[type].some(function(id){ return lu[type].indexOf(id) === -1; });
  });
}

function osRedacMajClignotement(){
  var win = _windows['redactions'];
  var fenetreVisible = !!win && !(win.el && win.el.classList.contains('minimized'));
  var doitClignoter = !fenetreVisible && osRedacResteDuNonLu();
  var icone = document.querySelector('#os-dock .dock-icon[data-page="redactions"]');
  if(icone) icone.classList.toggle('notif-clignote', doitClignoter);
}

function osRedacChargerNonLus(){
  var uid = getUserId();
  ...
  function ids(r){ return r.json().then(function(rows){
    return Array.isArray(rows) ? rows.map(function(x){ return x.id; }) : [];
  }); }
  fetch(SB_URL+'/rest/v1/membres_redactions?membre_id=eq.'+encodeURIComponent(uid)+'&select=redaction_id', {headers:authH})
  .then(function(r){ return r.json(); })
  .then(function(liens){
    var mesRedacIds = (Array.isArray(liens) ? liens : []).map(function(l){ return l.redaction_id; });
    return Promise.all([
      // Sujets de MES rédactions uniquement — même règle que osSujetsCharger
      mesRedacIds.length
  ...
  .then(function(res){
    _redacNonLusIds = { sujets:res[0], recrutement:res[1], cps:res[2] };
    osRedacMajClignotement();
  })
  .catch(function(){});
}

function osRedacClignotementInit(){
  if(_redacClignoteInit) return;
  _redacClignoteInit = true;
  osRedacChargerNonLus();
  setInterval(osRedacChargerNonLus, 180000);
  setInterval(osRedacMajClignotement, 3000);
}

// ===== MES ARTICLES : CLIGNOTEMENT VERT DE L'ICÔNE (article à corriger) =====
// Même principe que Ma rédac ci-dessus, mais l'état n'est pas un "lu/non lu" local : il
// suit la base — l'icône clignote tant qu'au moins un article en relecture m'est
// assigné comme correcteur, et s'éteint tout seul une fois corrigé. Fenêtre Mes
// articles ouverte à l'écran = pas de clignotement.
var _corrClignoteInit = false;
var _corrAArticleACorriger = false;

function osMesArticlesMajClignotement(){
  var win = _windows['mes-articles'];
  var fenetreVisible = !!win && !(win.el && win.el.classList.contains('minimized'));
  var icone = document.querySelector('#os-dock .dock-icon[data-page="mes-articles"]');
  if(icone) icone.classList.toggle('notif-clignote-vert', !fenetreVisible && _corrAArticleACorriger);
}

function osMesArticlesChargerACorriger(){
  var uid = getUserId();
  if(!uid || uid === 'undefined' || !_session || !_session.access_token) return;
  var authH = Object.assign({}, SB_HEADERS, {'Authorization':'Bearer '+_session.access_token});
  fetch(SB_URL+'/rest/v1/articles?correcteur_id=eq.'+encodeURIComponent(uid)+'&statut=eq.en-relecture&select=id&limit=1', {headers:authH})
  .then(function(r){ return r.json(); })
  .then(function(rows){
...
function osMesArticlesClignotementInit(){
  if(_corrClignoteInit) return;
  _corrClignoteInit = true;
  osMesArticlesChargerACorriger();
  setInterval(osMesArticlesChargerACorriger, 90000);
  setInterval(osMesArticlesMajClignotement, 3000);
}

// Charger les notifs au démarrage
function osChargerNotifsDemarrage(){
  var role = getUserRole();
  // Clignotement de l'icône Ma rédac s'il y a du non lu
  setTimeout(osRedacClignotementInit, 2000);
  // Clignotement vert de Mes articles s'il y a un article à corriger
  setTimeout(osMesArticlesClignotementInit, 2500);
  // Badge CPs non lus
...
function osInitSujets(){
  osSujetsCharger(function(sujets){
    ...
    // "Prévenu" (toast + centre de notifs, une seule fois) est distinct de "lu" :
    // compo_os_sujets_vus ne doit être écrit que quand on a réellement vu le sujet dans
    // Ma rédac (voir _osRedacMarquerSujetsLus), sinon l'icône du dock ne pourrait jamais
    // clignoter pour un nouveau sujet — il était marqué vu dès le démarrage.
    var prevenus = [];
    try { prevenus = JSON.parse(localStorage.getItem('compo_os_sujets_prevenus') || '[]'); } catch(e){}
    var nouveaux = sujets.filter(function(s){ return seen.indexOf(s.id) === -1 && prevenus.indexOf(s.id) === -1; });
    if(!nouveaux.length) return;

    localStorage.setItem('compo_os_sujets_prevenus', JSON.stringify(prevenus.concat(nouveaux.map(function(s){ return s.id; }))));
...
function osParamsRenderSection(section, container){
  ...
  if(section === 'profil'){
    ...
    rowAvatar.innerHTML = '<div style="margin-bottom:0.6rem;"><div class="params-label">Choisir mon avatar</div><div class="params-label-sub">Visible partout où tu apparais dans Compo (organigramme, listes de membres…)</div></div>';
    ...
    // Taille du texte
    var rowTaille = document.createElement('div');
    rowTaille.className = 'params-row';
    var tailleActuelle = parseInt(localStorage.getItem('compo_os_taille_texte'),10) || 100;
    rowTaille.innerHTML = '<div><div class="params-label">Taille du texte</div><div class="params-label-sub">Agrandit tout le texte de l\'interface</div></div>';
    var tailleAction = document.createElement('div');
    tailleAction.style.cssText = 'display:flex;gap:0.4rem;flex-wrap:wrap;';
    [100,115,130,150].forEach(function(pct){
      var btn = document.createElement('button');
      btn.style.cssText = (tailleActuelle===pct ? accStyleActif : accStyleInactif);
      btn.textContent = (pct===100?'Normal':pct+'%');
      btn.onclick = function(){
        osAppliquerTailleTexte(pct);
        osParamsRenderSection('accessibilite', document.getElementById('params-content-area'));
      };
      tailleAction.appendChild(btn);
    });
    rowTaille.appendChild(tailleAction);
    container.appendChild(rowTaille);

    // Police adaptée
    var rowPolice = document.createElement('div');
    rowPolice.className = 'params-row';
    rowPolice.style.flexDirection = 'column';
    rowPolice.style.alignItems = 'flex-start';
    var policeActuelle = localStorage.getItem('compo_os_police') || 'defaut';
    rowPolice.innerHTML = '<div style="margin-bottom:0.6rem;"><div class="params-label">Police adaptée</div><div class="params-label-sub">Luciole (malvoyance) et OpenDyslexic (dyslexie) sont deux polices conçues spécifiquement pour ça</div></div>';
    var policeAction = document.createElement('div');
    policeAction.style.cssText = 'display:flex;gap:0.5rem;flex-wrap:wrap;';
    [
      {id:'defaut', label:'Par défaut', ff:'inherit'},
      {id:'luciole', label:'Luciole', ff:"'Luciole',sans-serif"},
      {id:'opendyslexic', label:'OpenDyslexic', ff:"'OpenDyslexic',sans-serif"}
    ].forEach(function(p){
      var btn = document.createElement('button');
      btn.style.cssText = (policeActuelle===p.id ? accStyleActif : accStyleInactif) + 'font-family:'+p.ff+' !important;';
      btn.textContent = p.label;
      btn.onclick = function(){
        osAppliquerPolice(p.id);
        osParamsRenderSection('accessibilite', document.getElementById('params-content-area'));
      };
      policeAction.appendChild(btn);
    });
    rowPolice.appendChild(policeAction);
    container.appendChild(rowPolice);

    // Mode malvoyant — contraste renforcé, cibles plus grandes, mise en page simplifiée.
    // Premier passage limité à 6 écrans (Ma rédac, Agenda, Menu démarrer, Écran de
    // connexion, Mes articles, Rédiger) — les autres suivront dans un second temps.
    var rowMalvoyant = document.createElement('div');
    rowMalvoyant.className = 'params-row';
    var malvoyantActif = localStorage.getItem('compo_os_mode_malvoyant') === '1';
    rowMalvoyant.innerHTML = '<div><div class="params-label">Mode malvoyant <span class="badge-beta">Bêta</span></div><div class="params-label-sub">Contraste renforcé, boutons plus grands, mise en page simplifiée — disponible pour l\'instant sur Ma rédac, l\'agenda, le menu démarrer, l\'écran de connexion, Mes articles et Rédiger</div></div>';
    var malvoyantAction = document.createElement('div');
    malvoyantAction.style.cssText = 'display:flex;gap:0.4rem;';
    var btnMalvNormal = document.createElement('button');
    btnMalvNormal.style.cssText = !malvoyantActif ? accStyleActif : accStyleInactif;
    btnMalvNormal.textContent = 'Normal';
    var btnMalvActif = document.createElement('button');
    btnMalvActif.style.cssText = malvoyantActif ? accStyleActif : accStyleInactif;
    btnMalvActif.textContent = '◐ Activé';
    btnMalvNormal.onclick = function(){
      osAppliquerModeMalvoyant(false);
      osParamsRenderSection('accessibilite', document.getElementById('params-content-area'));
    };
    btnMalvActif.onclick = function(){
      osAppliquerModeMalvoyant(true);
      osParamsRenderSection('accessibilite', document.getElementById('params-content-area'));
    };
    malvoyantAction.appendChild(btnMalvNormal);
    malvoyantAction.appendChild(btnMalvActif);
    rowMalvoyant.appendChild(malvoyantAction);
    container.appendChild(rowMalvoyant);
  }
...
// Mode malvoyant : contraste renforcé + cibles plus grandes + mise en page simplifiée —
// voir la classe :root.mode-malvoyant dans le <style>, juste après .mode-leger.
function osAppliquerModeMalvoyant(actif){
  var root = document.documentElement;
  if(actif){
    localStorage.setItem('compo_os_mode_malvoyant', '1');
    root.classList.add('mode-malvoyant');
  } else {
    localStorage.removeItem('compo_os_mode_malvoyant');
    root.classList.remove('mode-malvoyant');
  }
}
function osAppliquerModeMalvoyantAuDemarrage(){
  osAppliquerModeMalvoyant(localStorage.getItem('compo_os_mode_malvoyant') === '1');
}

// ===== ACCESSIBILITÉ =====
// Police adaptée : 'defaut' | 'luciole' | 'opendyslexic' — voir les classes CSS
// :root.accessibilite-* juste après .mode-leger dans le <style>.
function osAppliquerPolice(id){
  var root = document.documentElement;
  root.classList.remove('accessibilite-luciole','accessibilite-opendyslexic');
  if(id === 'luciole') root.classList.add('accessibilite-luciole');
  else if(id === 'opendyslexic') root.classList.add('accessibilite-opendyslexic');
  if(id && id !== 'defaut') localStorage.setItem('compo_os_police', id);
  else localStorage.removeItem('compo_os_police');
}
function osAppliquerPoliceAuDemarrage(){
  osAppliquerPolice(localStorage.getItem('compo_os_police') || 'defaut');
}

// Taille du texte : un pourcentage appliqué à la racine — comme la quasi-totalité des
// tailles de police dans Compo sont en rem (relatives à la racine), ça suffit à tout
// agrandir proportionnellement sans devoir toucher aux centaines de style="" du JS.
function osAppliquerTailleTexte(pct){
  document.documentElement.style.fontSize = pct + '%';
  if(pct && pct !== 100) localStorage.setItem('compo_os_taille_texte', pct);
  else localStorage.removeItem('compo_os_taille_texte');
}
function osAppliquerTailleTexteAuDemarrage(){
  osAppliquerTailleTexte(parseInt(localStorage.getItem('compo_os_taille_texte'),10) || 100);
}
